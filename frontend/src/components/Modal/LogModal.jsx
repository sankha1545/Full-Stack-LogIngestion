  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 p-4 backdrop-blur-sm" onClick={onClose}>
      <motion.div
        ...
        className="w-full max-w-4xl overflow-hidden rounded-[28px] border border-slate-800 bg-slate-950 text-slate-100 shadow-2xl"
      >
        <div className="flex items-center justify-between border-b border-slate-800 px-6 py-4">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="rounded-full bg-slate-800 px-2.5 py-1 text-xs text-slate-300">{kind}</span>
            <span className="text-sm text-slate-400">{formatLogTimestamp(log.timestamp)}</span>
          </div>
          <button onClick={onClose} className="rounded-xl p-2 hover:bg-slate-900">
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className="space-y-6 p-6">
          <div>
            <div className="mb-2 text-xs uppercase tracking-[0.2em] text-slate-500">Message</div>
            <div className="rounded-[20px] border border-slate-800 bg-slate-900/80 p-4 text-sm leading-7 text-slate-100">{log.message}</div>
          </div>
          ...
          <div>
            <div className="mb-2 flex items-center justify-between">
              ...
              <button onClick={() => copyValue(JSON.stringify(metadata, null, 2))} className="inline-flex items-center gap-1 text-xs text-sky-300 hover:text-sky-200">
                <Copy className="h-3.5 w-3.5" /> Copy JSON
              </button>
            </div>
            <pre className="max-h-72 overflow-auto rounded-[20px] border border-slate-800 bg-slate-900/80 p-4 text-xs leading-6 text-emerald-300">{JSON.stringify(metadata, null, 2)}</pre>
          </div>
        </div>
      </motion.div>
    </div>
  ...
  return (
    <div className="rounded-[20px] border border-slate-800 bg-slate-900/60 p-4">
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="break-all text-sm text-slate-100">{value || "NA"}</div>
        {value && <button onClick={() => onCopy(value)} className="text-xs text-sky-300">Copy</button>}
      </div>
    </div>
  );
}
